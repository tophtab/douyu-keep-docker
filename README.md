...
| `ui.themeMode` | WebUI 主题模式：`light`、`dark`、`system` |
| `collectGift.cron` | 领取任务的 cron 表达式（6位，含秒），默认 `0 0 0 * * *` 表示每天零点 |
...
- **概览页** — 首页优先展示登录状态、领取任务、保活赠送、双倍赠送、最近日志和快捷操作
- **独立栏目** — 左侧拆分为概览、登录与领取、保活赠送、双倍赠送、粉丝牌同步、运行日志
- **独立领取任务** — 领取荧光棒拥有单独 cron、单独状态和手动触发入口
- **粉丝牌驱动同步** — 保活和双倍都围绕同一份粉丝牌列表同步配置，不再手动导入保活房间
- **粉丝牌状态** — 独立页面自动展示粉丝牌列表，并检查每个房间当前是否开启双倍
